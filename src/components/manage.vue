  ...
  <div class="manage">
    ...
    <div class="viewImgBox" v-if="showImgCtrl">
      <div class="imgContainer" @click="showImgCtrl = false">
        <div class="inner" v-for="(item,index) in viewList">
      ...
      handleClick(row) {
        ...
        this.$api.admin.getJf({
          ...
          if(res.data.code === 200){
            ...
            this.$alert('积分：' + res.data.data.jifen.response.point, '用户积分', {confirmButtonText: '确定',});
          }else{
            this.$vux.toast.show({
              type: 'text',
              text: '获取商城数据失败'
